...
- Nice paper on QGIS and its architecture `r Citep(my_bib, "graser_processing_2015", .opts = list(cite.style = "authoryear"))`.
...
.pull-right[
```{r, echo=FALSE, message=FALSE}
int = st_read(file.path(tempdir(), "int.shp"), quiet = TRUE)
plot(poly_1$geometry, xlim = c(-1, 1), ylim = c(-1, 1))
plot(st_geometry(poly_2), add = TRUE)
...
- Nice paper on SAGA, it's history and architecture `r Citep(my_bib, "conrad_system_2015", .opts = list(cite.style = "authoryear"))`
- [https://geocompr.robinlovelace.net/gis.html](https://geocompr.robinlovelace.net/gis.html)
