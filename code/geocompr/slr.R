...
# sf::st_write(scz, "pres/geocompr/scz-osm.geojson")
scz = sf::read_sf("pres/geocompr/scz-osm.geojson")
...
# sf::write_sf(scz_mapedit, "pres/geocompr/scz-mapedit.geojson")
scz_mapedit = sf::read_sf("pres/geocompr/scz-mapedit.geojson")
plot(scz_mapedit)
scz_sp = as(scz_mapedit, "Spatial")
# e = elevatr::get_elev_raster(locations = scz_sp, z = 5, src = "aws")
# raster::writeRaster(x = e, filename = "pres/geocompr/scz-elev-z5.tif")
e = raster("pres/geocompr/scz-elev-z5.tif")
...
plot(e1p[1:2, ], cex = 5)
st_distance(e1p[1, ], e1p[2, ]) # 1.37 km res. (x), 1.49 km res (y) Phew!
st_distance(e1p[1, ], e1p[3, ]) # 1.37 km res. (x), 1.49 km res (y) Phew!
...
writeRaster(e_low, "pres/geocompr/scz-elev-low.tif")

system.time({
  r_orig = raster::raster("~/data/aster/20180816184319_429977190-14-degrees-polish-coast.tif")
  plot(r_orig)
  })
r_agg = aggregate(r, 100)
res(r_agg)
res(e)
plot(r_agg)
plot(e_low, add = T)
...
m2
tmap_save(m, "/home/robin/repos/geostats_18/pres/geocompr/scz-map.html")
browseURL("pres/geocompr/scz-map.html")

# elevation data around prague
prague = osmdata::getbb("prague", format_out = "sf_polygon")
plot(prague)
write_sf(prague, "pres/geocompr/prague.geojson")
prague_elev = get_elev_raster(as(prohonice, "Spatial"), z = 9, src = "aws")
writeRaster(prague_elev, "pres/geocompr/prague_elev.tif")
